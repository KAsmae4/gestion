...
type RestOf<T extends IDemande | NewDemande> = Omit<T, 'dateDemande' | 'dateDebut' | 'dateFin'> & {
...
export class DemandeService {
  ...
  findAllByEtudiantId(id: number): Observable<EntityArrayResponseType> {
    return this.http
      .get<RestDemande[]>(`${this.resourceUrl}/etudiant/${id}`, { observe: 'response' })
      .pipe(map(res => this.convertResponseArrayFromServer(res)));
  }

  findAllNouvelle(): Observable<EntityArrayResponseType> {
    return this.http
      .get<RestDemande[]>(`${this.resourceUrl}/nouvelle`, { observe: 'response' })
      .pipe(map(res => this.convertResponseArrayFromServer(res)));
  }
  ...
  protected convertDateFromClient<T extends IDemande | NewDemande | PartialUpdateDemande>(demande: T): RestOf<T> {
    return {
      ...demande,
      dateDemande: demande.dateDemande?.format(DATE_FORMAT) ?? null,
      dateDebut: demande.dateDebut?.format(DATE_FORMAT) ?? null,
      dateFin: demande.dateFin?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restDemande: RestDemande): IDemande {
    return {
      ...
      dateDebut: restDemande.dateDebut ? dayjs(restDemande.dateDebut) : undefined,
      dateFin: restDemande.dateFin ? dayjs(restDemande.dateFin) : undefined,
    };
  }
