...
type RestOf<T extends IEtudiant | NewEtudiant> = Omit<T, 'dateNaissance'> & {
...
export class EtudiantService {
  ...
  search(query: string): Observable<EntityArrayResponseType> {
    ...
    options = options.append('query', query);

    return this.http
      .get<RestEtudiant[]>(this.resourceUrl + '/search', { params: options, observe: 'response' })
      .pipe(map(res => this.convertResponseArrayFromServer(res)));
  }
  ...
  protected convertDateFromClient<T extends IEtudiant | NewEtudiant | PartialUpdateEtudiant>(etudiant: T): RestOf<T> {
    return {
      ...etudiant,
      dateNaissance: etudiant.dateNaissance?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restEtudiant: RestEtudiant): IEtudiant {
    return {
      ...restEtudiant,
      dateNaissance: restEtudiant.dateNaissance ? dayjs(restEtudiant.dateNaissance) : undefined,
    };
  }
